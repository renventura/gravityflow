...
<svg width="100%" height="100%" viewBox="0 20 581 640" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;">
    <g id="Layer 1" transform="matrix(1,0,0,1,-309.5,-180)">
		...
		/**
		 * Logs debug messages to the Gravity Flow log file generated by the Gravity Forms Logging feature.
		 *
		 * @since 2.2.3-dev
		 *
		 * @param string     $calling_method The calling method name or an empty string to have the name retrieved using debug_backtrace().
		 * @param string     $message        The message to be logged.
		 * @param null|mixed $var            Null or the variable to be appended after the message.
		 */
		public function log( $calling_method, $message, $var = null ) {
			if ( empty( $calling_method ) ) {
				$calling_method = $this->get_log_calling_method();
			}

			$line = sprintf( '%s(): %s', $calling_method, $message );
			...
			if ( ! is_null( $var ) ) {
				...
				$line .= $this->get_log_formatted_var( $var );
			}
		...
		/**
		 * Return the name of the method which called the logging method.
		 *
		 * @since 2.2.3-dev
		 *
		 * @return string
		 */
		public function get_log_calling_method() {
			$backtrace    = debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS );
			$logger_found = false;

			foreach ( $backtrace as $item ) {
				if ( in_array( $item['function'], array( 'log', 'log_debug', 'log_error' ) ) ) {
					$logger_found = true;
					continue;
				}
		...
		/**
		 * Formats the variable which is to be appended to the logging statement.
		 *
		 * @since 2.2.3-dev
		 *
		 * @param mixed $var The variable to be formatted for inclusion in the log.
		 *
		 * @return string
		 */
		public function get_log_formatted_var( $var ) {
			$output = array();
			...
			switch ( gettype( $var ) ) {
				case 'object':
					if ( defined( 'WP_DEBUG' ) && WP_DEBUG || isset( $_GET['gform_debug'] ) ) {
						...
						$output['label'] = $var->label;
					} else {
						$callback = array( $var, 'get_log_formatted_var' );
				...
				case 'array':
					$properties = array( 'id', 'date_created', 'date_updated', 'form_id', 'title', 'to', 'subject' );

					foreach ( $properties as $property ) {
						if ( ! empty( $var[ $property ] ) ) {
							$output[ $property ] = $var[ $property ];
						}
					}
					break;
			}
